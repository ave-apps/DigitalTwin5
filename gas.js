...
// Versie: april 2026 — inclusief telefoonbeheer + mobiel medewerker + metingen
// ══════════════════════════════════════════════════════════════════════════════
...
function handleRequest(params, body) {
  ...
  try {
    ...
    // ── Metingen (rap-app → DT5) ─────────────────────────────────────────
    else if (action === 'pushMeting')         result = handlePushMeting(body);
    else if (action === 'pullMetingen')       result = handlePullMetingen(params);
...
function handlePushDienst(body) {
  ...
  const sheet = getOrCreateSheet('InDienst', [
    ...
    'mobiel','mobielZichtbaar','tijdUit','dienstType','collegaIds'
  ]);
  const data  = sheet.getDataRange().getValues();
...
function handlePullDienst() {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    records.push({
      ...
      collegaIds:      row[15] ? String(row[15]).split(',').filter(Boolean) : []
    });
  }
...
function ensureTelefoonSheet() {
  ...
  if (!sheet) {
    ...
    ['0-A','0-B','1-A','1-B','2-A','2-B','3-A','3-B'].forEach(s => {
...
// ══════════════════════════════════════════════════════════════════════════════
// METINGEN — rap-app schrijft, DT5 leest
// Kolommen: id · kamer · datum · dienst · ts · auteur ·
//           tempL · tempR · spo2 · tensieS · tensieD · hartritme · opgeslagenOp
// pullMetingen geeft per kamer de LAATSTE meting van vandaag terug
// ══════════════════════════════════════════════════════════════════════════════

function handlePushMeting(body) {
  const m = body && body.meting;
  if (!m || !m.kamer) return { ok: false, error: 'Geen meting of kamer' };

  const sheet = getOrCreateSheet(SHEET_METINGEN, [
    'id','kamer','datum','dienst','ts','auteur',
    'tempL','tempR','spo2','tensieS','tensieD','hartritme','opgeslagenOp'
  ]);

  // Genereer id als niet meegestuurd
  const id = m.id || (m.kamer + '_' + m.datum + '_' + m.ts + '_' + (m.auteur||'x')).replace(/[^a-zA-Z0-9_\-\.]/g, '');

  // Verwijder eventuele bestaande rij met zelfde id (update)
  const data = sheet.getDataRange().getValues();
  for (let i = data.length - 1; i >= 1; i--) {
  ...
  sheet.appendRow([
    ...
    m.datum        || '',
    m.dienst       || '',
    // ts als tekst opslaan: prefix apostrof voorkomt dat Sheets het als tijd-waarde opslaat
    "'" + (m.ts   || ''),
    m.auteur       || '',
    m.tempL        || '',
    m.tempR        || '',
...
function handlePullMetingen(params) {
  const datum = params && params.datum;
  const today = datum || Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd');
  ...
  // Verzamel per kamer alle metingen van vandaag, bewaar de laatste (meest recente ts)
  const perKamer = {};
  for (let i = 1; i < data.length; i++) {
    const row = data[i];
    if (!row[0]) continue;
    const rowDatum = row[2] instanceof Date
      ? Utilities.formatDate(row[2], Session.getScriptTimeZone(), 'yyyy-MM-dd')
      : String(row[2]).substring(0, 10);
    if (rowDatum !== today) continue;

    const kamer = String(row[1]);
    const ts    = String(row[4]).replace(/^'/, ''); // strip apostrof-prefix

    // Bewaar de meting met de laatste ts (vergelijk als string HH:MM — werkt voor zelfde dag)
    if (!perKamer[kamer] || ts > perKamer[kamer].ts) {
      perKamer[kamer] = {
        id:        String(row[0]),
        kamer,
        datum:     rowDatum,
        ...
        tempR:     String(row[7]  || ''),
        spo2:      String(row[8]  || ''),
        tensieS:   String(row[9]  || ''),
        tensieD:   String(row[10] || ''),
        hartritme: String(row[11] || '')
      };
    }
  }

  return { ok: true, metingen: perKamer };
}
...
function ensureAllSheets() {
  ...
  getOrCreateSheet(SHEET_METINGEN, [
    'id','kamer','datum','dienst','ts','auteur',
    'tempL','tempR','spo2','tensieS','tensieD','hartritme','opgeslagenOp'
  ]);
  ensureTelefoonSheet();
...
// Test: run handmatig in GAS editor
function testPullDienst() {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('InDienst');
  if(!sheet) { Logger.log('Geen InDienst sheet'); return; }
  const data  = sheet.getDataRange().getValues();
  const today = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd');
  Logger.log('Vandaag (GAS): ' + today);
  Logger.log('Aantal rijen: ' + (data.length - 1));
  for(let i = 1; i < Math.min(data.length, 8); i++) {
    const row = data[i];
    const datum = row[10] instanceof Date
      ? Utilities.formatDate(row[10], Session.getScriptTimeZone(), 'yyyy-MM-dd')
      : String(row[10]).substring(0, 10);
    Logger.log('Rij ' + i + ': datum=' + datum + ' id=' + row[0] + ' match=' + (datum === today));
  }
}
